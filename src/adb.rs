...
impl AdbHeader {
    ...
    pub fn to_bytes(&self) -> [u8; 24] {
        let mut arr = [0u8; 24];
        arr[0..4].copy_from_slice(&self.command.to_le_bytes());
    ...
    pub fn from_bytes(data: &[u8]) -> AdbHeader {
        let command = u32::from_le_bytes(data[0..4].try_into().unwrap());
...
impl Decoder for AdbServerCodec {
    ...
    fn decode(&mut self, src: &mut bytes::BytesMut) -> Result<Option<Self::Item>> {
        println!("decode server message: {}", pretty_hex::pretty_hex(&src));
...
impl Encoder<AdbPacket> for AdbServerCodec {
    ...
    fn encode(&mut self, packet: AdbPacket, dst: &mut bytes::BytesMut) -> Result<()> {
        ...
        println!("encode server message:\n{}", pretty_hex::pretty_hex(&dst));
